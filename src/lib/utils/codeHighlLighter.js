 ...
 */
 async function highlighter(code, lang, meta) {
    ...
    const metaMatch = meta && meta.match(/\(([^)]+)\)/); // Ищем название блока кода в круглых скобках

    if (metaMatch) {
        title = metaMatch[1].trim(); // Получаем название блока кода
        const metaWithoutTitle = meta.replace(metaMatch[0], ''); // Удаляем название блока кода из meta
        meta = metaWithoutTitle.trim(); // Устанавливаем обновленный meta
    }
    ...
    // Добавляем название блока кода перед блоком кода
    if (title) {
